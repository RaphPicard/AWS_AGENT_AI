...
# Denies discussions about some products
# Blocks requests and responses containing harmful content
# Blocks requests containing certain words
...
bedrock_runtime = boto3.client('bedrock-runtime')   # client to test guardrail directly without going through the agent
...
def test_agent_with_guardrail(guardrail_id, guardrail_version, prompt):
    ...
    bedrock_model = BedrockModel(
        model_id="us.anthropic.claude-haiku-4-5-20251001-v1:0", # j'ai changé pour que ca marche 
        region_name=region,
        guardrail_id=guardrail_id,          # dans le modèle on spécifie le guardrail à utiliser
