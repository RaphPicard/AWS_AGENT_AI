#AWS Console ==> Cloud Formation ==> Stacks ==> cfn ==> Outputs
# cfn-BedrockAgentCoreRoleName - Name of the IAM Role for the AgentCore Gateway
# cfn-CognitoDiscoveryUrl - Cognito Discovery URL for authentication
# cfn-RetrieveProductReviewsFunctionArn - ARN of the Lambda function that fetches product reviews
# cfn-CognitoClientId - Cognito User Pool Client Id for authentication

# cfn-BedrockAgentCoreRoleName : cfn-bedrock-agentcore-role
# cfn-CognitoDiscoveryUrl : https://cognito-idp.us-west-2.amazonaws.com/us-west-2_YBcZCKFN9/.well-known/openid-configuration
# cfn-RetrieveProductReviewsFunctionArn : arn:aws:lambda:us-west-2:969581442868:function:cfn-retrieve-product-reviews
# cfn-CognitoClientId : 601ep3kebv2o41jobmf6qgsqa4
...
from mcp.client.streamable_http import streamablehttp_client    #obsolete : la laisser quand meme selon claude...
...
ssm_client.put_parameter(   # avoiding an error (choose the right Model value)
    Name='prod_search_agent_model_id',
    Value='us.anthropic.claude-haiku-4-5-20251001-v1:0',    #changer lui
    Type='String',
    Overwrite=True
)


# SSM (AWS Console : Systems Manager ==> Application Tools ==> Parameter Store) is a service that allows you to store and manage configuration data and secrets. Here we are using it to store the FAQ Knowledge Base ID securely, instead of hardcoding it in the script.
...
# AWS Console ==> Amazon Bedrock AgentCore ==> Gateway 
# Gateway URL : https://gateway-pd-search-3i6jwil1zz.gateway.bedrock-agentcore.us-west-2.amazonaws.com/mcp
...
def get_products_from_kb(user_query: str) -> str:
    ...
    try:        
        ...
        result = retrieve.retrieve(tool_use)        # The retrieve tool returns a list of results with "status" and "content". We will check the status and return the content if successful.
...
def retrieve_product_details(input_query):
    ...
    try:
        with streamable_http_mcp_client:
            ...
            product_search_results = str(product_search_agent(input_query)) #apply the agent to the input query and get the results
